...
if _azure_endpoint:
    llm_manager.azure_endpoint = _azure_endpoint
...
async def trigger_term_extraction(
    ...
    # Load project-specific prompt template and LLM params (or defaults)
    ...
    llm_p = settings.llm_params or DEFAULT_LLM_PARAMS
    ...
    for file in target_files:
        ...
        for chunk in chunks:
            ...
            try:
                response_text = await generate_with_timeout_multi(
                    prompt, provider=model,
                    temperature=llm_p.get("temperature"), max_tokens=llm_p.get("max_tokens"), top_p=llm_p.get("top_p"),
                )
...
async def extract_definitions(project_id: int, term_id: int = Form(...), model: str = Form("gemini-1.5-pro"), db: Session = Depends(get_research_db)):
    ...
    # Load project-specific prompt templates and LLM params
    settings = _get_project_settings(project_id, db)
    templates = settings.prompt_templates or DEFAULT_PROMPT_TEMPLATES
    llm_p = settings.llm_params or DEFAULT_LLM_PARAMS
    ...
    for p_type, p_text in prompts.items():
        try:
            definition = await generate_with_timeout_multi(
                p_text, provider=model,
                temperature=llm_p.get("temperature"), max_tokens=llm_p.get("max_tokens"), top_p=llm_p.get("top_p"),
            )
...
Output ONLY the definition text."""
        ...
        definition_krag = await generate_with_timeout_multi(
            keyword_rag_prompt, provider=model,
            temperature=llm_p.get("temperature"), max_tokens=llm_p.get("max_tokens"), top_p=llm_p.get("top_p"),
        )
...
Output ONLY the definition text."""
        ...
        definition_vrag = await generate_with_timeout_multi(
            vector_rag_prompt, provider=model,
            temperature=llm_p.get("temperature"), max_tokens=llm_p.get("max_tokens"), top_p=llm_p.get("top_p"),
        )
...
async def get_project_settings(project_id: int, db: Session = Depends(get_research_db)):
    ...
    return {
        ...
        "llm_params": settings.llm_params or DEFAULT_LLM_PARAMS,
        "defaults": {
            "prompt_templates": DEFAULT_PROMPT_TEMPLATES,
            "parser_config": DEFAULT_PARSER_CONFIG,
            "llm_params": DEFAULT_LLM_PARAMS,
...
async def update_project_settings(project_id: int, request: Request, db: Session = Depends(get_research_db)):
    ...
    if "llm_params" in body:
        settings.llm_params = body["llm_params"]
...
async def neurosymbolic_define(
    ...
    # Load project parser config and LLM params
    settings = _get_project_settings(project_id, db)
    parser_cfg = settings.parser_config or DEFAULT_PARSER_CONFIG
    llm_p = settings.llm_params or DEFAULT_LLM_PARAMS
