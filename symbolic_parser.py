...
class SymbolicDefinitionParser:
    ...
    def validate_structure(self, definition: str, term: str, lang: str = "en") -> dict:
        ...
        # ── 1. Circularity check ──
        # Detects multiple forms of circularity:
        #   a) Direct: definition starts with the term
        #   b) Full-phrase: the complete term appears as a phrase in the definition
        #   c) Morphological: derivations of the term's content words appear
        #      (e.g. "disambiguate" / "disambiguating" in a def of "disambiguation")
        if rc("circularity")["enabled"]:
            ...
            full_term_in_def = bool(re.search(term_pattern, definition_lower))

            # Morphological circularity: extract stems (≥5 chars) from
            # content words in the term, then look for them in the definition.
            # Skips short function words (the, of, a, in, for, ...).
            morpho_hits = []
            _stop_words = {"the", "a", "an", "of", "in", "for", "to", "and", "or",
                           "by", "on", "at", "is", "are", "was", "its", "with", "from",
                           "that", "this", "as", "be", "it", "not", "but", "has", "have",
                           "no", "do", "does", "been", "had", "may", "can", "will",
                           "ο", "η", "το", "του", "της", "των", "στο", "στη", "στον",
                           "και", "με", "από", "για", "σε", "ένα", "μια"}
            for tw in term_words:
                if tw in _stop_words or len(tw) < 4:
                    continue
                # Use the first 5+ chars as a stem (covers most inflections)
                stem_len = max(5, len(tw) - 3)
                stem = tw[:stem_len]
                # Find words in the definition that share this stem
                stem_pat = r"\b" + re.escape(stem) + r"\w*\b"
                matches = re.findall(stem_pat, definition_lower)
                # Filter out exact matches of the original word (those are fine
                # only if they're short function-like words — but we already
                # skipped those above). Also skip if the match IS the term word
                # itself and it didn't appear as the full phrase.
                for m in matches:
                    if m != tw:  # derivation, not the exact same word
                        morpho_hits.append((tw, m))
            ...
            elif full_term_in_def:
                checks.append({"name": "circularity", "passed": False, "weight": w,
                    "reason": f"Circular: the full term '{term}' appears within its own definition"})
            elif morpho_hits:
                examples = ", ".join(f"'{hit}' (from '{src}')" for src, hit in morpho_hits[:3])
                checks.append({"name": "circularity", "passed": False, "weight": w,
                    "reason": f"Morphological circularity: derived forms found in definition: {examples}"})
            else:
