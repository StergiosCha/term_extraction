...
class MultiLLMManager:
    ...
    async def generate_with_provider(
        ...
        # Use caller-provided values or fall back to model config defaults
        effective_max_tokens = max_tokens or config["max_tokens"]

        try:
            if provider_type == "gemini":
                return await self._generate_gemini(prompt, api_key, model_id, timeout, temperature, effective_max_tokens, top_p)
            elif provider_type == "anthropic":
                return await self._generate_claude(prompt, api_key, model_id, timeout, effective_max_tokens, temperature, top_p)
            elif provider_type == "openai":
                return await self._generate_openai(prompt, api_key, model_id, timeout, effective_max_tokens, temperature, top_p)
    ...
    async def _generate_gemini(self, prompt: str, api_key: str, model_id: str, timeout: int,
        ...
        def call_gemini():
            ...
            model = genai.GenerativeModel(model_id, generation_config=gen_config if gen_config else None)
            response = model.generate_content(prompt)
            return response.text

        return await asyncio.wait_for(loop.run_in_executor(None, call_gemini), timeout=timeout)

    async def _generate_claude(self, prompt: str, api_key: str, model_id: str, timeout: int, max_tokens: int,
    ...
    async def _generate_openai(self, prompt: str, api_key: str, model_id: str, timeout: int, max_tokens: int,
                               temperature: Optional[float] = None, top_p: Optional[float] = None) -> str:
        import openai
        loop = asyncio.get_event_loop()
    ...
    async def _generate_openrouter(self, prompt: str, api_key: str, model_id: str, timeout: int, max_tokens: int,
                                    temperature: Optional[float] = None, top_p: Optional[float] = None) -> str:
        import aiohttp
        payload = {
            "model": model_id,
    ...
    async def _generate_azure_openai(self, prompt: str, api_key: str, model_id: str, timeout: int, max_tokens: int,
                                      temperature: Optional[float] = None, top_p: Optional[float] = None) -> str:
        """Azure OpenAI Service (GPT models deployed via Azure AI Foundry)."""
        import openai
        loop = asyncio.get_event_loop()
...
async def generate_with_timeout_multi(
    prompt: str, provider: str = None, timeout: int = 30,
    session_id: Optional[str] = None,
    temperature: Optional[float] = None,
    max_tokens: Optional[int] = None,
    top_p: Optional[float] = None,
) -> str:
    """Generate with timeout using specified provider and optional LLM params."""
    if provider is None:
        provider = llm_manager.default_provider.value
    return await llm_manager.generate_with_provider(
        prompt, provider, timeout, session_id,
